...
```
처음에는 주어진 숫자를 문자열로 바꾸고 정렬했는데, 맨 앞 숫자가 같고 길이가 다른 경우를 생각해주는 조건이 없어 테케 통과가 안되었다.
문자열의 최대 길이가 4이므로 문자열의 최소 길이를 4로 만들어 정렬해줬다.

001 등 0이 제일 앞에 나오는 경우, 만든 문자열을 숫자로 바꾸고 다시 문자열로 바꿔 출력했다.
